...
namespace LipingShare.LCLib.Asn1Processor
{
	public class Asn1Node : IAsn1Node
	{
		...
		private string GetHexPrintingStr(Asn1Node startNode, string baseLine, string lStr, int lineLen)
		{
			...
			if (text2.Length > 0)
			{
				if (baseLine.Length + text2.Length < lineLen)
				{
					string text3 = text;
					text = text3 + baseLine + "'" + text2 + "'";
				}
				else
				{
					text = text + baseLine + FormatLineHexString(lStr, indentStr.Length, lineLen, text2);
				}
			}
			else
			{
		...
		private string FormatLineString(string lStr, int indent, int lineLen, string msg)
		{
			...
			for (int i = 0; i < msg.Length; i += num)
			{
				...
				{
					string text3 = text;
					text = text3 + "\r\n" + lStr + Asn1Util.GenStr(len, ' ') + "'" + msg.Substring(i, num) + "'";
				}
			}
			return text;
		}
		...
		public string GetText(Asn1Node startNode, int lineLen)
		{
			...
			switch (tag)
			{
				case 3:
					...
					if (text2.Length + text3.Length < lineLen)
					{
						if (text3.Length < 1)
						{
							text = text + text2 + "\r\n";
						...
						string text7 = text;
						text = text7 + text2 + "'" + text3 + "'\r\n";
					}
					else
					{
						text = text + text2 + FormatLineHexString("      |      |       | ", GetIndentStr(startNode).Length, lineLen, text3 + "\r\n");
					}
					break;
				case 6:
					{
						Oid oid = new Oid();
						string text5 = oid.Decode(new MemoryStream(data));
						string text6 = oid.GetOidName(text5);
						text += string.Format("{0,6}|{1,6}|{2,7}|{3} {4} : {5} [{6}]\r\n", dataOffset, dataLength, lengthFieldBytes, GetIndentStr(startNode), TagName, text6, text5);
						break;
					}
				case 13:
					{
						RelativeOid relativeOid = new RelativeOid();
						string text5 = relativeOid.Decode(new MemoryStream(data));
						string text6 = "";
						text += string.Format("{0,6}|{1,6}|{2,7}|{3} {4} : {5} [{6}]\r\n", dataOffset, dataLength, lengthFieldBytes, GetIndentStr(startNode), TagName, text6, text5);
						break;
					}
				...
				case 30:
					...
					{
						string text8 = text;
						text = text8 + text2 + "'" + text3 + "'\r\n";
					}
					else
					{
						text = text + text2 + FormatLineString("      |      |       | ", GetIndentStr(startNode).Length, lineLen, text3) + "\r\n";
					}
					break;
				case 2:
					...
					break;
				default:
					if ((tag & 0x1F) == 6 || Asn1Util.IsAsciiString(Data))
					{
						text2 = string.Format("{0,6}|{1,6}|{2,7}|{3} {4} : ", dataOffset, dataLength, lengthFieldBytes, GetIndentStr(startNode), TagName);
						text3 = Asn1Util.BytesToString(data);
						if (text2.Length + text3.Length < lineLen)
		...
		public string GetLabel(uint mask)
		{
			...
			switch (tag)
			{
				case 3:
					{
						...
						string text6 = text;
						text = text6 + " " + TagName + " UnusedBits: " + unusedBits;
						if ((mask & 2u) != 0)
						{
				...
				default:
					...
					if ((mask & 2u) != 0)
					{
						text2 = (((tag & 0x1F) != 6 && !Asn1Util.IsAsciiString(Data)) ? Asn1Util.ToHexString(data) : Asn1Util.BytesToString(data));
						text += ((text2.Length > 0) ? (" : '" + text2 + "'") : "");
					}
					break;
			}
